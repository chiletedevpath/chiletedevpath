# Política Editorial de Chilete DevPath

Chilete DevPath publica contenido con honestidad técnica. Cada guía, ejercicio, laboratorio, solución, ficha académica o proyecto debe ayudar a la comunidad a aprender con claridad, respetando la autoría y diferenciando el contenido propio del contenido inspirado o basado en fuentes externas.

Esta política aplica a todo el ecosistema:

- `aprendizaje`
- `academia`
- `colaboraciones`
- `portafolio`
- repositorios académicos y personales publicados bajo Chilete DevPath

## Políticas complementarias

- [Política de Uso Responsable de IA](POLITICA_USO_RESPONSABLE_IA.md)
- [Política de Bienestar en el Aprendizaje Técnico](POLITICA_BIENESTAR_APRENDIZAJE.md)

## Principios

- El contenido propio se identifica como material creado para Chilete DevPath.
- El contenido inspirado en fuentes externas se presenta como adaptación, no como copia.
- Las fuentes externas se citan cuando se usan ideas, ejercicios, documentación, videos, libros, cursos o repositorios.
- Las evaluaciones internas, materiales privados o enunciados cerrados no se publican completos.
- El trabajo de otras personas no se presenta como propio.
- La IA puede apoyar la estructuración, revisión o generación de ideas, pero el contenido publicado debe ser comprendido, revisado y validado por el autor de Chilete DevPath.
- Cada material debe poder explicarse con palabras propias antes de publicarse.

## Cómo se presenta el contenido

| Tipo de contenido | Presentación en Chilete DevPath |
|---|---|
| Creado para Chilete DevPath | Se indica que el ejercicio, guía o laboratorio fue creado para esta ruta. |
| Inspirado en una fuente | Se menciona la fuente y se aclara que el contenido fue adaptado. |
| Basado en documentación oficial | Se enlaza la documentación consultada. |
| Basado en una clase o curso | Se registra la institución, curso o contexto cuando se puede compartir públicamente. |
| Relacionado con una evaluación o material privado | No se publica el enunciado completo; se crea una versión propia o se documentan solo aprendizajes generales. |
| Proyecto colaborativo | Se reconoce el repositorio, autoría o equipo original cuando corresponda. |

## Fuentes y referencias

Cuando un material use una fuente externa, incluye una sección llamada `Fuentes` o `Referencias`. Esa sección permite que la comunidad revise el origen del contenido y continúe estudiando desde la fuente original.

Formato usado:

```md
## Fuentes

- Autor o institución: título del recurso. URL o referencia consultada.
```

Si el material fue adaptado, se presenta así:

```md
## Fuentes

- Inspirado en: nombre del recurso o curso. Adaptado con ejercicios propios para Chilete DevPath.
```

## Uso de IA

Cuando se use IA para apoyar una explicación, crear ejercicios o revisar redacción, el material debe aclarar que fue revisado y validado antes de publicarse. El criterio completo se define en la [Política de Uso Responsable de IA](POLITICA_USO_RESPONSABLE_IA.md).

Formato usado:

```md
## Nota de autoría

Contenido creado para Chilete DevPath con apoyo de IA para estructuración o revisión. El criterio, selección y validación final corresponden al autor de Chilete DevPath.
```

## Material académico

Los proyectos, prácticas y evidencias académicas se documentan con cuidado:

- Se puede explicar el aprendizaje obtenido.
- Se puede crear una versión propia de un ejercicio.
- Se puede enlazar un repositorio propio publicado.
- No se deben publicar enunciados privados completos, rúbricas cerradas o material interno de evaluación si no corresponde compartirlo públicamente.

## Colaboraciones

Cuando un proyecto pertenezca a otra persona, equipo o repositorio externo, Chilete DevPath debe presentarlo como colaboración. La documentación debe reconocer el contexto, el repositorio original y el alcance de la participación.

## Compromiso con la comunidad

Chilete DevPath busca compartir contenido útil, honesto y verificable. Si un ejercicio, explicación, laboratorio o proyecto no puede explicarse con palabras propias, todavía no está listo para publicarse como contenido formativo.
